cd '/Volumes/SynMap Data/2024-07-30 SynMap Thalamus/Registered';
...
for i = 1:size(file_names, 1)
    ...
    sub_responders = find(responders.slice == i+2);
    ...
    TSData = load(fullfile(fn, ts_files(ts_idx(1))));
    ts_data = horzcat(ts_data, cell2mat(TSData.ts));
    TSData2 = load(fullfile(fn, ts_files(ts_idx(2))));
    ts_data = horzcat(ts_data, cell2mat(TSData2.ts));
    size(ts_data, 2)

    optos = regexp(ts_files, '^Opto');
    opto_idx = find(~cellfun(@isempty,optos));
    stimData = load(fullfile(fn, ts_files(opto_idx(1))));
    opto_data = vertcat(opto_data, find(stimData.stim == 1));
    stimData2 = load(fullfile(fn, ts_files(opto_idx(2))));
    opto_data = vertcat(opto_data, find(stimData2.stim == 1));
    ...
    ROIData = load(fullfile(fn, ts_files(roi_idx(1))));
    keepIDs = find(ismember(sub_ids, ROIData.roiIndices));
    keepIDs = sub_ids(keepIDs);
    %keepIDs = keepIDs + 1;
    roi_data = vertcat(roi_data, ROIData.roiIndices);
    ROIData2 = load(fullfile(fn, ts_files(roi_idx(2))));
    keepIDs2 = find(ismember(sub_ids, ROIData2.roiIndices));
    keepIDs2 = sub_ids(keepIDs2);
    %keepIDs2 = keepIDs2 + 1;
    try
        if size(ROIData2.roiIndices, 2) > size(ROIData2.roiIndices, 1)
            roi_data = vertcat(roi_data, ROIData2.roiIndices');
        else
            roi_data = vertcat(roi_data, ROIData2.roiIndices);
        end
    catch
        display("This didn't work")
        continue
    end
    
    display(size(roi_data, 1))

    cheriff_pos = vertcat(cheriff_pos, direct_anno.UserLabel(keepIDs), direct_anno.UserLabel(keepIDs2));
    cher = regexp(ts_files, 'CheRiff_Input');
    cher_idx = find(~cellfun(@isempty,cher));
    cheriff = readtable(fullfile(fn, ts_files(cher_idx(2))));
    cheriff_data = vertcat(cheriff_data, cheriff.normAllLayers(keepIDs), cheriff.normAllLayers(keepIDs2));
    cheriff_abs = vertcat(cheriff_abs, cheriff.allLayers(keepIDs), cheriff.allLayers(keepIDs2));
    L1_cheriff = vertcat(L1_cheriff, cheriff.normL1(keepIDs), cheriff.normL1(keepIDs2));
    soma_cheriff = vertcat(soma_cheriff, cheriff.normAtSoma(keepIDs), cheriff.normAtSoma(keepIDs2));

    classif_data = vertcat(classif_data, sub_supertype(keepIDs), sub_supertype(keepIDs2));
    subclass_data = vertcat(subclass_data, sub_subclass(keepIDs), sub_subclass(keepIDs2));
    classif_cols = vertcat(classif_cols, sub_cols(keepIDs), sub_cols(keepIDs2));
    slice_data = vertcat(slice_data, sub_slice(keepIDs), sub_slice(keepIDs2));
    slice_depth = vertcat(slice_depth, sub_slice_depth(keepIDs), sub_slice_depth(keepIDs2));
    depth_data = vertcat(depth_data, sub_depth(keepIDs), sub_depth(keepIDs2));
...
%     typeData = cells(:,1:(ceil(numel(optoix)/2)),:); % & responders.ResponseSNR > 0
...
% 
%     else
%         plot(smtrace, 'color', cols, 'LineWidth', 1.5); xlim([0 180]); ylim([-1, 2]); title(strcat(uniqueTypes{i}, ', n=', num2str(size(smtrace, 2))));
%     end
% end
% 
...
%% Save the response data with all associated metadata
...
save('Responses_by_type_0730.mat', 'to_write')
