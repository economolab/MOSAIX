cd '/Volumes/SynMap Data/2024-08-02 SynMap Thalamus/Registered';
...
for i = 1:size(file_names, 1)
    ...
    if i == 2 
        sub_responders = find(responders.slice == i+1);
        ...
        TSData = load(fullfile(fn, ts_files(ts_idx(2))));
        if i == 1
            ts_data = cell2mat(TSData.ts);
        else
            ts_data = horzcat(ts_data, TSData.ts);
        end
        size(ts_data, 2)
        
        optos = regexp(ts_files, '^Opto');
        opto_idx = find(~cellfun(@isempty,optos));
        stimData = load(fullfile(fn, ts_files(opto_idx(2))));
...
save('Responses_by_type_0802.mat', 'to_write')
